...
const EditCategory = () => {
      ...
      if (response.data && response.data.category) {
        ...
        setError(
          response.data?.message ||
            "Failed to update category due to an unknown reason."
        );
        toast.error(
          response.data?.message ||
            "Failed to update category due to an unknown reason."
  ...
  if (error && !categoryName) {
    // Only show full error if we failed to load initial data
    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-100 p-4">
      <div className="w-full max-w-2xl bg-white shadow-xl rounded-lg p-8">
        <div className="mb-6 flex justify-between items-center">
          <h2 className="text-2xl font-bold text-gray-800 nunito">
            Edit Category
          </h2>
          <button
            onClick={() => navigate(-1)}
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div>
            ...
            <div className="flex items-center gap-4 mb-4">
              ...
              {imagePreview && (
                <div className="relative">
                  ...
                  />
                  <span className="absolute top-0 right-0 bg-purple-500 text-white text-xs px-1 rounded-bl-md">
                    New Preview
                  </span>
                </div>
              )}
            ...
            <p className="text-sm text-gray-500 mt-1">
              Upload a new image to replace the current one. Leave blank to keep
              existing.
            </p>
          </div>
